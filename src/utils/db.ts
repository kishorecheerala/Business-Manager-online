    ...
    for (const storeName of STORE_NAMES) {
        ...
        for (const item of remoteItems) {
            if (item && item.id) {
                ...
                // Special Case: Profile Overwrite
                // If we are merging the 'profile' store, and the local profile appears to be the default "Skipped" profile,
                ...
                // If local item DOES exist, we KEEP the local item (Local Wins), unless it's the default profile we want to replace.
                if (!localItem || shouldOverwrite) {
