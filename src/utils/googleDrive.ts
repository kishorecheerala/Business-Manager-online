...
export const debugDriveState = async (accessToken: string) => {
    ...
    try {
        ...
        // 1. Check Folder
        log("Searching for App Folder...");
        const folders = await getCandidateFolders(accessToken);
        
        if (folders.length === 0) {
            log(`❌ No folder found named '${APP_FOLDER_NAME}'`);
            return { logs, details };
        }
        ...
        for (const folder of folders) {
            log(`Checking Folder: ${folder.name} (${folder.id})`);
            
            // List files in this folder
            const q = `'${folder.id}' in parents and trashed=false`;
            const response = await fetch(`https://www.googleapis.com/drive/v3/files?q=${encodeURIComponent(q)}&fields=files(id,name,mimeType,size,modifiedTime)&orderBy=modifiedTime desc`, {
                headers: getHeaders(accessToken)
            });
            
            if (!response.ok) {
                log(`❌ Failed to list files: ${response.status}`);
                continue;
            }
            ...
            files.forEach((f: any) => {
                log(`   - ${f.name} (${(Number(f.size)/1024).toFixed(1)} KB)`);
            });
        }
