...
const Calendar: React.FC<CalendarProps> = ({ value, onChange }) => {
    ...
    const daysInMonth = useMemo(() => {
        ...
        for (let i = firstDay; i > 0; i--) {
            days.push({ day: prevMonthLastDate - i + 1, isCurrentMonth: false });
        }
        ...
        // Next month's days to fill grid (42 cells total for 6 rows)
    ...
    const handlePrevMonth = () => {
        setDisplayDate(new Date(Date.UTC(displayDate.getUTCFullYear(), displayDate.getUTCMonth() - 1, 1)));
    };
