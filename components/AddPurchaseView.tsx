                        ...
                        {items.map(item => (
                            <div key={item.productId} className="bg-gray-50 dark:bg-slate-700/50 rounded border dark:border-slate-700 overflow-hidden animate-slide-up-fade">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-sm p-2 border-t dark:border-slate-600">
                                    <div>
                                        <FormattedNumberInput label="Quantity" value={item.quantity} onChange={e => handleItemUpdate(item.productId, 'quantity', parseFloat(e.target.value))} placeholder="Qty" />
                                    </div>
                                    <div>
                                        <FormattedNumberInput label="Buy Price" value={item.price} onChange={e => handleItemUpdate(item.productId, 'price', parseFloat(e.target.value))} placeholder="Buy Price" />
                                    </div>
                                    <div>
                                        <FormattedNumberInput label="Sale Price" value={item.saleValue} onChange={e => handleItemUpdate(item.productId, 'saleValue', parseFloat(e.target.value))} placeholder="Sale Price" />
                                    </div>
                                    <div>
