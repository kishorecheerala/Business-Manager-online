...
const MenuPanel: React.FC<MenuPanelProps> = ({ isOpen, onClose, onProfileClick, onNavigate, onOpenDevTools, onLockApp, onOpenChangeLog, onOpenSignIn, onHelpClick, onOpenAPIConfig, onOpenNavCustomizer, onOpenSecuritySettings }) => {
    ...
    useEffect(() => {
        ...
        window.addEventListener('OPEN_DATA_IMPORT', handler);
        return () => window.removeEventListener('OPEN_DATA_IMPORT', handler);
    }, []);
    ...
    const handleRemoveFont = async (e: React.MouseEvent, id: string) => {
        e.stopPropagation();
    ...
    if (!isOpen && !isAuditOpen && !isCloudDebugOpen && !isColorPickerOpen && !isHeaderColorPickerOpen && !isGradientPickerOpen && !isPinModalOpen && !isInvoiceSettingsOpen && !isUISettingsOpen && !isImportOpen && !isAboutDevOpen) return null;
    ...
    return (
        <>
            ...
            <AboutDeveloperModal isOpen={isAboutDevOpen} onClose={() => setIsAboutDevOpen(false)} />
            ...
            {isOpen && (
                <>
                    <div className="fixed inset-0 z-[89] bg-transparent" onClick={onClose}></div>
                    <div
                        className="fixed top-16 left-4 mt-2 w-80 bg-white/95 dark:bg-slate-800/95 backdrop-blur-xl rounded-2xl shadow-2xl border border-gray-200 dark:border-slate-700 text-text dark:text-slate-200 animate-scale-in origin-top-left z-[90] flex flex-col overflow-hidden max-h-[calc(100vh-6rem)]"
                        role="dialog"
                    ...
                    >
                        ...
                        <div className="flex-grow overflow-y-auto custom-scrollbar min-h-0 overscroll-contain pb-10">
                            ...
                            <div className="p-2 space-y-1 pb-3">
                                ...
                                <button onClick={() => { onClose(); setIsAboutDevOpen(true); }} className="menu-item text-emerald-600 dark:text-emerald-400">
                                    <Code className="w-5 h-5" />
                                    <span className="flex-grow text-sm font-medium">About Developer</span>
                                    <ChevronRight className="w-4 h-4 text-gray-400" />
                                </button>
                                                ...
                                                {group.colors.map((t) => {
                                                    ...
                                                    return (
                                                        <button
                                                            ...
                                                            className={`w-9 h-9 rounded-full flex items-center justify-center transition-all relative ${isSelected
                                            ...
                                            {GRADIENT_PRESETS.map((g) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center transition-all relative ${isSelected
                                                            ? 'ring-2 ring-offset-2 ring-gray-400 dark:ring-gray-500 scale-110 shadow-md'
                                                            : 'border border-gray-200 dark:border-slate-600 opacity-90 hover:opacity-100 hover:scale-105'
                                                            }`}
