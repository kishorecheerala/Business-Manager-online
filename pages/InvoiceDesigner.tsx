...
import { dummyCustomer, dummySale, defaultLabels, REPORT_SCENARIOS, ReportScenarioKey, PRESETS } from '../utils/invoiceDefaults';
import FormattedNumberInput from '../components/FormattedNumberInput';

// pdfjs-dist removed (moved to PDFCanvasPreview component)
...
// Lazy load the heavy PDF Preview component (uses pdfjs-dist)
const PDFCanvasPreview = React.lazy(() => import('../components/invoice/PDFCanvasPreview'));
...
const InvoiceDesigner: React.FC<InvoiceDesignerProps> = ({ setIsDirty, setCurrentPage }) => {
    ...
    // --- Custom Section & Drag/Drop Handlers ---
    ...
    const handleDragOver = (e: React.DragEvent, index: number) => {
        ...
        const currentLayout = configRef.current.layout;
        const newOrder = [...(currentLayout.sectionOrdering || [])];
        const draggedItem = newOrder[draggingIndex];
        newOrder.splice(draggingIndex, 1);
        newOrder.splice(index, 0, draggedItem);

        handleBatchLayoutChange({ sectionOrdering: newOrder });
        setDraggingIndex(index);
    };
    ...
    const addCustomSection = (type: CustomSection['type']) => {
        ...
        const newSection: CustomSection = {
            id: newId,
            type,
            content: type === 'text-block' ? 'New Text Section' : undefined,
            styles: { fontSize: 10, align: 'center', marginTop: 2, marginBottom: 2, height: type === 'divider' ? 5 : 30 }
        };

        const currentLayout = configRef.current.layout;
        const newCustomSections = [...(currentLayout.customSections || []), newSection];
        const newOrder = [...(currentLayout.sectionOrdering || []), newId];

        handleBatchLayoutChange({
            customSections: newCustomSections,
            sectionOrdering: newOrder
        });
        setActiveSectionId(newId);
    };

    const removeSection = (index: number, id: string) => {
        const currentLayout = configRef.current.layout;
        const newOrder = [...(currentLayout.sectionOrdering || [])];
        newOrder.splice(index, 1);

        let newCustomSections = currentLayout.customSections;
        if (id.startsWith('custom-')) {
            newCustomSections = (newCustomSections || []).filter(s => s.id !== id);
    ...
    const updateCustomSection = (id: string, updates: Partial<CustomSection>) => {
        const currentLayout = configRef.current.layout;
        const newCustomSections = (currentLayout.customSections || []).map(s => s.id === id ? { ...s, ...updates } : s);
    ...
    return (
        <div className="flex flex-col md:flex-row h-full bg-gray-50 dark:bg-slate-950 overflow-hidden font-sans relative">
                        ...
                        <div className="space-y-6 animate-fade-in-fast">
                            ...
                            <div className="space-y-3 border-t dark:border-slate-800 pt-4">
                                <div className="flex justify-between items-center">
                                    <label className="text-xs font-bold text-slate-500 uppercase block">Section Order & Components</label>
                                    <div className="flex gap-2">
                                        <button onClick={() => addCustomSection('text-block')} className="px-2 py-0.5 rounded border bg-white dark:bg-slate-700 text-[10px] text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50 flex items-center gap-1"><Plus size={10} /> Text</button>
                                        <button onClick={() => addCustomSection('image-block')} className="px-2 py-0.5 rounded border bg-white dark:bg-slate-700 text-[10px] text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50 flex items-center gap-1"><Plus size={10} /> Image</button>
                                        <button onClick={() => addCustomSection('divider')} className="px-2 py-0.5 rounded border bg-white dark:bg-slate-700 text-[10px] text-indigo-600 dark:text-indigo-300 hover:bg-indigo-50 flex items-center gap-1"><Plus size={10} /> Divider</button>
                                    </div>
                                </div>
                                <div className="space-y-1">
                                    {(localConfig.layout.sectionOrdering || ['header', 'title', 'details', 'table', 'totals', 'terms', 'signature', 'footer']).map((section, idx, arr) => {
                                        ...
                                        const customData = isCustom ? (localConfig.layout.customSections || []).find(s => s.id === section) : null;
                                        let label = section;
                                        if (isCustom) {
                                            if (customData?.type === 'text-block') label = `Text: ${customData.content?.substring(0, 15) || 'Empty'}...`;
                                            else if (customData?.type === 'divider') label = 'Divider Line';
                                            else if (customData?.type === 'image-block') label = 'Custom Image';
                                        }
                                        ...
                                        return (
                                            <div
                                                ...
                                                onClick={() => isCustom && setActiveSectionId(isActive ? null : section)}
                                                className={`flex items-center justify-between p-2 border rounded text-xs capitalize cursor-grab active:cursor-grabbing transition-colors ${isActive ? 'bg-indigo-50 border-indigo-200 dark:bg-indigo-900/20 dark:border-indigo-700' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700'
                                                    } ${draggingIndex === idx ? 'opacity-50' : ''}`}
                                            >
                                                <div className="flex items-center gap-2">
                                                    <GripVertical size={12} className="text-slate-400" />
                                                    <span className={`font-medium ${isActive ? 'text-indigo-700 dark:text-indigo-300' : 'text-slate-700 dark:text-slate-300'}`}>
                                                        {label}
                                                    </span>
                                                </div>
                                                <div className="flex gap-1 items-center">
                                                    {isCustom ? (
                                                        <button onClick={async (e) => {
                                                            e.stopPropagation();
                                                            if (await showConfirm("Remove this section?", { variant: 'danger' })) {
                                                                removeSection(idx, section);
                                                            }
                                                        }} className="p-1 hover:bg-red-50 dark:hover:bg-red-900/30 text-red-500 rounded"><Trash2 size={12} /></button>
                                                    ) : (
                                                        <span className="text-[9px] text-slate-400 uppercase tracking-tighter">Fixed</span>
                                                    )}
                                                </div>
                                            </div>
                                        );
                                ...
                                {/* Custom Section Editor Panel */}
                                {activeSectionId && (() => {
                                    const section = (localConfig.layout.customSections || []).find(s => s.id === activeSectionId);
                                    if (!section) return null;

                                    return (
                                        <div className="p-3 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg animate-fade-in-fast mt-2">
                                            <div className="flex justify-between items-center mb-2">
                                                <span className="text-[10px] font-bold text-indigo-700 dark:text-indigo-400 uppercase">Edit Component</span>
                                            </div>
                                            {section.type === 'text-block' && (
                                                <div className="space-y-2">
                                                    <textarea
                                                        ...
                                                        className="w-full text-xs p-2 border rounded dark:bg-slate-900 dark:border-slate-700 dark:text-white"
                                                        rows={3}
                                                        placeholder="Enter text..."
                                                    />
                                                    <div className="flex gap-2">
                                                        <div className="flex-1">
                                                            <span className="text-[9px] text-slate-500 block">Align</span>
                                                            <div className="flex border rounded dark:border-slate-700 overflow-hidden">
                                                                {['left', 'center', 'right'].map((align) => (
                                                                    <button
                                                                        key={align}
                                                                        onClick={() => updateCustomSection(section.id, { styles: { ...section.styles, align: align as any } })}
                                                                        className={`flex-1 p-1 flex justify-center ${section.styles?.align === align ? 'bg-indigo-100 text-indigo-600' : 'bg-white dark:bg-slate-800'}`}
                                                                    >
                                                        ...
                                                        <div className="w-16">
                                                            ...
                                                            <FormattedNumberInput
                                                                value={section.styles?.fontSize || 10}
                                                                onChange={(e) => updateCustomSection(section.id, { styles: { ...section.styles, fontSize: parseInt(e.target.value) } })}
                                                                className="w-full text-xs p-1 border rounded dark:bg-slate-900 dark:border-slate-700"
                                                            />
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div className="space-y-2">
                                                    {section.content ? (
                                                        <div className="relative group">
                                                            <img src={section.content} className="h-20 object-contain bg-white border rounded mx-auto" alt="Custom" />
                                                            <button onClick={async () => {
                                                                if (await showConfirm("Remove this image?", { variant: 'danger' })) {
                                                                    updateCustomSection(section.id, { content: undefined });
                                                                }
                                                            }} className="absolute top-1 right-1 bg-red-500 text-white rounded-full p-1 opacity-0 group-hover:opacity-100"><Trash2 size={10} /></button>
                                                        </div>
                                                    ) : <div className="h-20 bg-white dark:bg-slate-900 border border-dashed flex items-center justify-center text-[10px] text-gray-400">No Image Selected</div>}
                                                    <input type="file" accept="image/*" onChange={(e) => {
                                                        const file = e.target.files?.[0];
                                                        if (file) {
                                                            const reader = new FileReader();
                                                            reader.onload = (ev) => updateCustomSection(section.id, { content: ev.target?.result as string });
                                                            reader.readAsDataURL(file);
                                                        }
                                                    }} className="text-[10px] w-full" />
                                                    <div>
                                                        <span className="text-[9px] text-slate-500 block">Height (mm)</span>
                                                        <input
                                                            type="range" min="10" max="100"
                                                            value={section.styles?.height || 30}
                                                            onChange={(e) => updateCustomSection(section.id, { styles: { ...section.styles, height: parseInt(e.target.value) } })}
                                            ...
                                            {section.type === 'divider' && (
                                                <div>
                                                    <span className="text-[9px] text-slate-500 block">Vertical Spacing (mm)</span>
                                                    <input
                                                        type="range" min="2" max="50"
                                                        value={section.styles?.height || 5}
                                                        onChange={(e) => updateCustomSection(section.id, { styles: { ...section.styles, height: parseInt(e.target.value) } })}
                                                        className="w-full h-1"
                                                    />
                                                </div>
                                ...
                                <div className="p-3 bg-gray-50 dark:bg-slate-800 rounded border dark:border-slate-700 mb-3">
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        {localConfig.layout.backgroundImage && (
                                            <button
                                                onClick={async () => {
                                                    if (await showConfirm("Remove background image?", { variant: 'danger' })) {
                                                        handleConfigChange('layout', 'backgroundImage', undefined);
                                                    }
                                                }}
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <FormattedNumberInput
                                            value={localConfig.fonts.headerSize}
                                    ...
                                    <div>
                                        ...
                                        <FormattedNumberInput
                                            value={localConfig.fonts.bodySize}
            ...
            <main className={`flex-1 flex flex-col min-w-0 min-h-0 bg-gray-100 dark:bg-slate-900/50 relative overflow-hidden z-10 ${mobileActiveView === 'design' ? 'hidden md:flex' : 'flex'} ${typeof window !== 'undefined' && window.innerWidth < 768 ? 'order-1' : ''}`}>
                ...
                {/* Canvas */}
                {/* Canvas */}
                <React.Suspense fallback={<div className="flex-1 flex items-center justify-center"><Loader2 className="w-8 h-8 animate-spin text-indigo-500" /></div>}>
                    <PDFCanvasPreview
                        config={localConfig}
